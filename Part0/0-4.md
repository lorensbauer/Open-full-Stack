...
sequenceDiagram
    ...
    browser->>server: POST https://studies.cs.helsinki.fi/exampleapp/new_notes (Form Data -> note: new note) 
    activate server
    Note right of server: Serve pushes note into <br> notes array 
    server-->>browser: Returns location: /notes
    deactivate server
